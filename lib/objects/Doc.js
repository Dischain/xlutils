...
class Doc {
   ...
   * @param {*} lowSigns
   */
  constructObjects(type, valuesRange, topSigns, midSigns, lowSigns) {
    ...
    if (type == 'plain') 
    {
      ...
      return xl.readFile(this._path, this._sheet).then((res) => {
        ...
        objsRange.forEach((cell) => {
          ...
          // if (!StringUtils.containsArrItem(cell.getValue(), topSigns) || !StringUtils.containsArrItem(cell.getValue(), midSigns)) {
    ...
    if (type == 'top')
    {
      ...
      return xl.readFile(this._path, this._sheet).then((res) => {
        ...
        this._topRows = {};
        // const topFG = this._ws.getCell(topSigns).fill.fgColor,
        //       midFG = this._ws.getCell(midSigns).fill.fgColor;
        ...
        objsRange.forEach((cell) => {
            ...
            if (StringUtils.containsArrItem(cell.getValue(), topSigns)) {
              ...
              //   (cell.getCell().fill.fgColor.theme == 0 && cell.getCell().fill.fgColor.tint == undefined)) return;
                ...
                const obj = 
                  new DocObject(cell.getAddress().toString(),
                              valuesRange,
                              this._ws, this._objectsRange, null, midSigns, topSigns);
    ...
    if (type == 'mid')
    {
      ...
      return xl.readFile(this._path, this._sheet).then((res) => {
        ...
        objsRange.forEach((cell) => {
          if (StringUtils.containsArrItem(cell.getValue(), midSigns)) {
            // const parentName = this._getParentByChildrenRange(cell.getAddress().toString());
            const obj = 
              new DocObject(cell.getAddress().toString(),
                            valuesRange,
                            this._ws, this._objectsRange, topSigns, lowSigns, midSigns);
