...
class RangeAddress extends Address{ 
  ...
  getUpperBound() {
    return REUtils.getUpperBound(this.address);
  }

  getLowerBound() {
    return REUtils.getLowerBound(this.address);
  }
  
  setUpperBound(uBound, validate) {
    if (validate) {
      if (REUtils.isAddress(uBound)) {
        this.address = REUtils.setUpperBound(this.address, uBound);
      } else {
  ...
  setLowerBound(lBound, validate) {
    if (validate) {
      if (REUtils.isAddress(lBound)) {
        this.address = REUtils.setLowerBound(this.address, lBound);
      } else {
